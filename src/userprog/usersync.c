...
void sys_lock_acquire(char* ptr) {
  ...
  for (struct list_elem* e = list_begin(&p->user_lock_list); e != list_end(&p->user_lock_list);
       e = list_next(e)) {
...
void sys_sema_up(char* ptr) {
  ...
  for (struct list_elem* e = list_begin(&p->user_sema_list); e != list_end(&p->user_sema_list);
       e = list_next(e)) {
